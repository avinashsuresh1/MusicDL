...
> * **`RELEASE_<version>` Branches**: Used for official production releases (e.g., [`RELEASE_1.1.7`](https://github.com/avinashsuresh1/MusicDL/tree/RELEASE_1.1.7), [`RELEASE_1.1.6`](https://github.com/avinashsuresh1/MusicDL/tree/RELEASE_1.1.6)). Created when changes are finalized, verified, and stable without regressions. Always use release branches for stable deployment.
...
1. Open your terminal in this folder and install dependencies (if not already installed):
   ```bash
   npm install
   ```
2. Compile and bundle the app:
   ```bash
   npm run tauri:build
   ```
   *(On Linux, this compiles `.deb`, `.rpm`, and packages `MusicDL.AppImage`. On Windows and macOS, it compiles native installers).*
3. Find the packaged outputs:
